...
local function LoadData()
    ...
    local data = union_db:lrange(relations_key,0,-1)
    for _,user_id in ipairs(data) do
        local user_key = "social:relations:"..user_id
        local info = union_db:hgetall(user_key)
        local temp = {}
        for i=1,#info,2 do
        ...
        relations[user_id] = temp
        name_to_id[temp.name] = user_id
    end
end

function command.Init()
    relations = {}
    union_db = redis.connect(sharedata.query("redis_conf_2"))
    LoadData()
end
...
function command.NewUser(user_id,name,role_id)
    ...
    union_db:lpush(all_relations_key,user_id)
    local user_key = "social:relations:"..user_id 
    union_db:hmset(user_key,"role_id",role_id,"name",name)
end
---------------------------------------------------------------
--更新玩家的登出时间
---------------------------------------------------------------
function command.UpdateLogoutTime(user_id,time)
    local info = relations[user_id]
    if info then
        info.logout_time = time
        local user_key = "social:relations:" .. user_id 
        union_db:hmset(user_key,"logout_time",time)
    end
end
---------------------------------------------------------------
--更新玩家的角色ID
...
function command.UpdateRoleID(user_id,role_id)
    ...
    if info then
        ...
        local user_key = "social:relations:" .. user_id 
        union_db:hmset(user_key,"role_id",role_id)
    end
...
function command.UpdateUserName(user_id,name)
    ...
    if info then
        info.name = name
        local user_key = "social:relations:" .. user_id 
        union_db:hmset(user_key,"name",name)
    end
end
---------------------------------------------------------------
--发送好友邀请
...
function command.MakeInvitation(src_user_id, dest_user_id)
    ...
    local src_friend_info = relations[src_user_id]
    if not src_friend_info then
        return "not_find_src_friend_info"
    end
    local dest_friend_info = relations[dest_user_id]
    ...
    union_db:hmset(user_key,"invitations",cjson.encode(dest_friend_info.invitations))
...
function command.AcceptInvite(src_user_id, dest_user_id)
    ...
    union_db:hmset(src_user_key,"friends",cjson.encode(src_friend_info.friends))

    local dest_user_key = "social:relations:" .. dest_user_id 
    union_db:hmset(dest_user_key,"friends",cjson.encode(dest_friend_info.friends),
    ...
    if dest_friend_info.invitations[src_user_id] then
        ...
        union_db:hmset(dest_user_key,"invitations",cjson.encode(dest_friend_info.invitations))
    end
    --通知对方,我已经同意加对方为好友
    local agent = skynet.call(".agent_manager", "lua", "GetAgentByUserId", dest_user_id)
    if agent then
...
function command.RefuseInvite(src_user_id, dest_user_id)
    ...
    local src_user_key = "social:relations:" .. src_user_id 
    union_db:hmset(src_user_key,"invitations",cjson.encode(src_friend_info.invitations))
