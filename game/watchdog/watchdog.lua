...
----------------------------------------------------------------------------
function watchdog:Init(gateserver)
    gate = gateserver
...
--解析接收到的数据
----------------------------------------------------------------------------
function watchdog:ProcessData(msg)
...
--客户端消息处理
----------------------------------------------------------------------------
function watchdog:OnReceiveData(fd,msg,ip)
...
--向客户端发送数据
----------------------------------------------------------------------------
function watchdog:SendToClient(fd,send_msg)
...
--处理登录
----------------------------------------------------------------------------
function watchdog:ProcessLogin(fd,data,ip)
    ...
    skynet.call(agent.service_id, "lua", "Start",gate,fd,ip,is_new_agent,user_id,server_id,platform,logintype,locale)
...
----------------------------------------------------------------------------
function watchdog:SetExpireTime(fd)
...
--将失效的agent加入重用池中
----------------------------------------------------------------------------
function watchdog:ReclaimAgent(agent)
...
--获取玩家的agent服务id
----------------------------------------------------------------------------
function watchdog:GetAgentByUserId(user_id)
    local agent = self.userid_to_agent[user_id]
    if agent then
...
local function SaveTimer()
    ...
    if not watchdog.userid_to_agent then
        return 
    end
    ...
    for user_id, agent in pairs(watchdog.userid_to_agent) do
    ...
    for user_id, agent in pairs(tmp_agent_map) do
        ...
        if not agent.lock then
            watchdog:ReclaimAgent(agent)
        end
    end
    watchdog.online_num = online_num
    --TODO:添加online记录
end
----------------------------------------------------------------------------
--整点调度
